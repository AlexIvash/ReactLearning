...
const Dialogs = (props) => {
    ...
    return (
        ...
        {/** какого-то хера необходимо обернуть все эти элементы в один div - иначе будет ошибка. То есть эти text-area и тд не могут быть не внутри div.*/}
            ...
            <input type="button" size="Large" className={styles.PostButton1} onClick={props.handleSendMessageClick} value="Send Message" />
            <input type="button" size="Large" className={styles.PostButton1} onClick={props.handleEditMessageClick} value="Edit Message" />
            <input type="button" size="Large" className={styles.PostButton2} onClick={props.handleDeleteMessageClick} value="Delete Message" />
        </div>
